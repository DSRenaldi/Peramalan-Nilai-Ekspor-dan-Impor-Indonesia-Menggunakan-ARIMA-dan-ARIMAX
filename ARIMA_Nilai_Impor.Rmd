...
```
Analisis:
Nilai p-value yg kurang dari 0.05 menandakan bahwa data berdistribusi normal
...
```
Analisis:
Nilai p-value yang lebih dari 0,05 menandakan bahwa tidak ada perbedaan yang signifikan antara rata-rata sampel dengan nilai yang diuji. 
...
```
Analisis:
Nilai p-value yang lebih dari 0,05 meandakan bahwa terdapat korelasi residual yang signifikan dalam model.

Peramalan
```{r}
(ramalan<- forecast::forecast(exp(ts(log.data.ts)),model=log.model.sarima442221_12, h=10))
```
Analisis:
Berdasarkan hasil peramalan diatas, dapat dilihat bahwa terjadi kenaikan nilai yang cukup signifikan untuk Nilai impor
```{r}
plot(ramalan)
```
Analisis:
Pada beberapa bulan ke depan, nilai impor (US) Indonesia akan mengalami kenaikan. Hal tersebut di karenakan kebanyakan bahan baku, bahan pokok Indonesia di impor dari luar negeri. Bahkan sampah pun kita impor dari luar negeri. Selain itu, Indonesia sedang mengembangkan bidang Electric Vehicle (EV). Untuk baterainya memang kita produksi dalam negeri, namun untuk komponen-komponen lain masih ada yang menggunakan produk luar. Tidak hanya itu, beberapa bahan pokok kita masih impor dari luar seperti beras, gandum, daging, susu, dll.
