...
```
Analisis:
Nilai p-value yang lebih dari 0,05 meandakan bahwa tidak ada korelasi residual yang signifikan dalam model.

Peramalan
```{r}
(ramalan<- forecast::forecast(exp(ts(log.data.ts)),model=log.model.sarima322231_12, h=10))
```
Analisis:
Berdasarkan hasil peramalan dapat dilihat bahwa akan terjadi sedikit kenaikan, namun nilainya cenderung turun

```{r}
plot(ramalan)
```
Analisis:
Nilai peramalan  Nilai Ekspor (US) untuk beberapa bulan ke depan cenderung mengalami penurunan. Meskipun pada 1-3 bulan kedepan nilainya sempat naik, akan tetapi saya simpulkan nilainya cenderung menurun untuk bulan-bulan berikutnya. Menurunnya nilai ekspor ini bisa jadi dikarenakan terjadinya resesi pada negara-negara di dunia. Resesi tersebut menyebabkan negara-negara penerima ekspor Indonesia mengurangi permintaannya.